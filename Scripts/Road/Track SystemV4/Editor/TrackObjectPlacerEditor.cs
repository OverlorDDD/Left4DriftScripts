    ...
    void OnSceneGUI()
    {
        ...
        foreach (var obj in tp.objects)
        {
            ...
            Handles.Label(pos + Vector3.up * sz * 2f,
                ...
                float arrowSz = HandleUtility.GetHandleSize(pos) * 0.7f;

EditorGUI.BeginChangeCheck();
Vector3 newPos = Handles.FreeMoveHandle(pos, arrowSz * 0.4f, Vector3.zero, Handles.SphereHandleCap);
if (EditorGUI.EndChangeCheck())
{
    Undo.RecordObject(tp, "Move Track Object");
    // Проєктуємо нову позицію назад на t і sideOffset
    float bestT = obj.t, bestDist = float.MaxValue;
    for (int s = 0; s < samples.Count; s += 4)
    {
        float dist = Vector3.SqrMagnitude(newPos - samples[s].position);
        if (dist < bestDist) { bestDist = dist; bestT = samples[s].t; }
    }
    obj.t = bestT;
    var closest = samples[Mathf.RoundToInt(bestT * (samples.Count-1))];
    obj.sideOffset = Vector3.Dot(newPos - closest.position, closest.right);
